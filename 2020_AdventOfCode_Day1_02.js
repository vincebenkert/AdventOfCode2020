...
for(var i=0; i<inputArray.length; i++){
	...
	for(var j=i+1; j<inputArray.length; j++){
           if(found)
             break;
		for(var k=i+2; k<inputArray.length; k++){
            if(found)
               break;
			sum = Number(inputArray[i]) + Number(inputArray[j]) + Number(inputArray[k]);
			//console.log("Current Sum = " + sum);  -- This takes a lot of time to write....
			if(sum == 2020){
			  output = Number(inputArray[i]) * Number(inputArray[j]) * Number(inputArray[k]);
			  console.log("The output value equals " + output);
			  found = true;
            }
        } 
    }
